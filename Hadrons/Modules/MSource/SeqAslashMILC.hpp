...
#ifndef Hadrons_MSource_SeqAslashMILC_hpp_
#define Hadrons_MSource_SeqAslashMILC_hpp_
...
MODULE_REGISTER_TMP(StagSeqAslashMILC, TSeqAslashMILC<FIMPL>, MSource);
...
void TSeqAslashMILC<FImpl>::execute(void)
{
    if (par().tA == par().tB)
    {
        LOG(Message) << "Generating AslashMILC sequential source(s) at t= " << par().tA 
		             << " using the photon field '" << par().emField 
                     << "'" << std::endl; 
    }
    else
    {
        LOG(Message) << "Generating AslashMILC sequential source(s) for "
                     << par().tA << " <= t <= " << par().tB 
		             << " using the photon field '" << par().emField 
                     << "'" << std::endl;
    }
...
#endif // Hadrons_MSource_SeqAslashMILC_hpp_
