...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(LoadCoarseEigenPackPar, std::string, filestem,
                                  bool, multiFile, unsigned int, sizeFine,
                                  unsigned int, sizeCoarse, bool, redBlack,
                                  unsigned int, Ls, std::string, blockSize,
                                  bool, orthogonalise, std::string, gaugeXform);
};
...
MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPack,
    ARG(TLoadCoarseEigenPack<
        CoarseFermionEigenPack<FIMPL, HADRONS_DEFAULT_LANCZOS_NBASIS>, GIMPL>),
    MIO);
MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPack200,
    ARG(TLoadCoarseEigenPack<CoarseFermionEigenPack<FIMPL, 200>, GIMPL>), MIO);
MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPack250,
    ARG(TLoadCoarseEigenPack<CoarseFermionEigenPack<FIMPL, 250>, GIMPL>), MIO);
MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPack400,
    ARG(TLoadCoarseEigenPack<CoarseFermionEigenPack<FIMPL, 400>, GIMPL>), MIO);
MODULE_REGISTER_TMP(
    StagLoadCoarseFermionEigenPack,
    ARG(TLoadCoarseEigenPack<
        CoarseFermionEigenPack<STAGIMPL, HADRONS_DEFAULT_LANCZOS_NBASIS>,
        GIMPL>),
    MIO);
MODULE_REGISTER_TMP(
    StagLoadCoarseFermionEigenPackF,
    ARG(TLoadCoarseEigenPack<
        CoarseFermionEigenPack<STAGIMPLF, HADRONS_DEFAULT_LANCZOS_NBASIS>,
        GIMPLF>),
    MIO);

MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPackF,
    ARG(TLoadCoarseEigenPack<
        CoarseFermionEigenPack<FIMPLF, HADRONS_DEFAULT_LANCZOS_NBASIS>,
        GIMPLF>),
    MIO);
MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPack200F,
    ARG(TLoadCoarseEigenPack<CoarseFermionEigenPack<FIMPLF, 200>, GIMPLF>),
    MIO);
MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPack250F,
    ARG(TLoadCoarseEigenPack<CoarseFermionEigenPack<FIMPLF, 250>, GIMPLF>),
    MIO);
MODULE_REGISTER_TMP(
    LoadCoarseFermionEigenPack400F,
    ARG(TLoadCoarseEigenPack<CoarseFermionEigenPack<FIMPLF, 400>, GIMPLF>),
    MIO);
...
void TLoadCoarseEigenPack<Pack, GImpl>::setup(void) {
  ...
  envCreateDerived(BasePack, Pack, getName(), par().Ls, par().sizeFine,
                   ...
                   envGetCoarseGrid(CoarseField, blockSize, par().Ls), gridIo,
                   gridCoarseIo);
...
void TLoadCoarseEigenPack<Pack, GImpl>::execute(void) {
  ...
  if (!par().gaugeXform.empty()) {
    LOG(Message) << "Applying gauge transformation to fine eigenvectors "
