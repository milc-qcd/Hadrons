/*
 * ImplicitlyRestartedLanczos.hpp, part of Hadrons
...
public:
  GRID_SERIALIZABLE_CLASS_MEMBERS(ImplicitlyRestartedLanczosPar, LanczosParams,
                                  lanczosParams, std::string, op, std::string,
                                  output, std::string, epackIn, bool, redBlack,
                                  bool, evenEigen, bool, multiFile);
};

template <typename Field, typename FieldIo = Field>
class TImplicitlyRestartedLanczos
    : public Module<ImplicitlyRestartedLanczosPar> {
public:
...
MODULE_REGISTER_TMP(FermionImplicitlyRestartedLanczos,
                    TImplicitlyRestartedLanczos<FIMPL::FermionField>, MSolver);
MODULE_REGISTER_TMP(
    FermionImplicitlyRestartedLanczosIo32,
    ARG(TImplicitlyRestartedLanczos<FIMPL::FermionField, FIMPLF::FermionField>),
    MSolver);
...
TImplicitlyRestartedLanczos<Field, FieldIo>::TImplicitlyRestartedLanczos(
...
TImplicitlyRestartedLanczos<Field, FieldIo>::getInput(void) {
...
template <typename Field, typename FieldIo>
std::vector<std::string>
TImplicitlyRestartedLanczos<Field, FieldIo>::getOutput(void) {
...
void TImplicitlyRestartedLanczos<Field, FieldIo>::setup(void) {
  LOG(Message) << "Setting up implicitly restarted Lanczos eigensolver for"
               << " operator '" << par().op << "' ("
               << par().lanczosParams.Nstop << " eigenvectors)..." << std::endl;
  ...
  envCreateDerived(BasePack, Pack, getName(), Ls, par().lanczosParams.Nm, grid,
                   gridIo);
  ...
  envTmp(ImplicitlyRestartedLanczos<Field>, "irl", Ls, chebyOp, hermOp,
         par().lanczosParams.Nstop, par().lanczosParams.Nk,
         par().lanczosParams.Nm, par().lanczosParams.resid,
         par().lanczosParams.MaxIt, par().lanczosParams.betastp,
...
void TImplicitlyRestartedLanczos<Field, FieldIo>::execute(void) {
  ...
  epack.record.operatorXml =
      vm().getModule(env().getObjectModule(par().op))->parString();
