...
protected:
    ...
    template <typename T = FImpl>
    IfNotStag<T,int> getNsc(void) const;

    template <typename T = FImpl>
    IfStag<T,int> getNsc(void) const;
...
public:
    // constructor/destructor
    SparseNoise(GridCartesian *g, const int nNoise, const int nSparseL, const int nSparseT=-1);
...
template <typename FImpl>
template <typename T>
IfNotStag<T,int> SpinColorDiagonalNoise<FImpl>::getNsc(void) const
{
    return Ns*FImpl::Dimension;
}
template <typename FImpl>
template <typename T>
IfStag<T,int> SpinColorDiagonalNoise<FImpl>::getNsc(void) const
{
...
{
    if(nSparseT == -1) nSparseT_ = nSparseL;
}
...
void SparseNoise<FImpl>::setProp(const int i)
{
    ...
    for(int d = 0; d < nd; ++d) {
        ...
        eta = where(coor_ == ((uint32_t) subdivs.rem), eta, 0.*eta);
