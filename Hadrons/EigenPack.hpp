...
void readElement(T &evec, RealD &eval, const unsigned int index,
  ...
  if (vecRecord.index != index) {
    HADRONS_ERROR(Io, "Eigenvector " + std::to_string(index) + " has a" +
                          " wrong index (expected " +
                          std::to_string(vecRecord.index) + ")");
  }
...
void writeElement(ScidacWriter &binWriter, T &evec, RealD &eval,
                  const unsigned int index, TIo *ioBuf, T *testBuf = nullptr) {
  VecRecord vecRecord;
...
static void writePack(const std::string filename, std::vector<T> &evec,
  ...
  if (multiFile) {
    ...
    for (int k = ki; k < kf; ++k) {
      ...
      writeElement(binWriter, evec[k - ki], eval[k - ki], k, ioBuf.get(),
                   testBuf.get());
      binWriter.close();
    }
  } else {
...
public:
  ...
  virtual void read(const std::string fileStem, const bool multiFile,
                    ...
                    const int traj = -1) {
    EigenPackIo::readPack<F, FIo>(this->evec, this->eval, this->record,
                                  evecFilename(fileStem, traj, multiFile), ki,
                                  kf, multiFile, gridIo_);
  }
...
template <typename FineF, typename CoarseF, typename FineFIo = FineF,
          typename CoarseFIo = CoarseF>
...
public:
  ...
  CoarseEigenPack(const size_t sizeFine, const size_t sizeCoarse,
    ...
    if (typeHash<FineF>() != typeHash<FineFIo>()) {
      if (gridFineIo == nullptr) {
        HADRONS_ERROR(Definition, "Fine I/O type different from vector type "
                                  "but null fine I/O grid passed");
      }
    }
    if (typeHash<CoarseF>() != typeHash<CoarseFIo>()) {
      if (gridCoarseIo == nullptr) {
        HADRONS_ERROR(Definition, "Coarse I/O type different from vector type "
                                  "but null coarse I/O grid passed");
      }
    }
  ...
  void readFine(const std::string fileStem, const bool multiFile,
                const int traj = -1) {
    EigenPack<FineF, FineFIo>::read(fileStem + "_fine", multiFile, traj);
  }

  void readCoarse(const std::string fileStem, const bool multiFile,
                  const int traj = -1) {
    PackRecord dummy;

    EigenPackIo::readPack<CoarseF, CoarseFIo>(
        evecCoarse, evalCoarse, dummy,
  ...
  virtual void read(const std::string fileStem, const bool multiFile,
    ...
    HADRONS_ERROR(Implementation,
                  "partial read not supported for CoarseEigenPack");
  }
  ...
  void writeFine(const std::string fileStem, const bool multiFile,
                 const unsigned int ki, const unsigned int kf,
                 const int traj = -1) {
    EigenPack<FineF, FineFIo>::write(fileStem + "_fine", multiFile, ki, kf,
                                     traj);
  }

  void writeCoarse(const std::string fileStem, const bool multiFile,
                   const int traj = -1) {
    EigenPackIo::writePack<CoarseF, CoarseFIo>(
        this->evecFilename(fileStem + "_coarse", traj, multiFile), evecCoarse,
        evalCoarse, this->record, evecCoarse.size(), multiFile, gridCoarseIo_);
  }
  ...
  void writeCoarse(const std::string fileStem, const bool multiFile,
    ...
    EigenPackIo::writePack<CoarseF, CoarseFIo>(
        this->evecFilename(fileStem + "_coarse", traj, multiFile), evecCoarse,
        evalCoarse, this->record, ki, kf, multiFile, gridCoarseIo_);
  }
...
using FermionEigenPack =
    EigenPack<typename FImpl::FermionField, typename FImplIo::FermionField>;

template <typename FImpl, int nBasis, typename FImplIo = FImpl>
using CoarseFermionEigenPack =
    CoarseEigenPack<typename FImpl::FermionField,
                    typename LocalCoherenceLanczos<typename FImpl::SiteSpinor,
                                                   typename FImpl::SiteComplex,
                                                   nBasis>::CoarseField,
                    typename FImplIo::FermionField,
                    typename LocalCoherenceLanczos<
                        typename FImplIo::SiteSpinor,
                        typename FImplIo::SiteComplex, nBasis>::CoarseField>;
